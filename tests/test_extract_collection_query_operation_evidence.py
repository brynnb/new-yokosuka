...
PATH = ROOT / "tools/scripting/operations/extract_collection_query_operation_evidence.py"
SPEC = importlib.util.spec_from_file_location("collection_query", PATH)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(MODULE)


class ExtractCollectionQueryOperationEvidenceTest(unittest.TestCase):
    def test_recovers_both_routes_and_all_calls(self):
        report = MODULE.build_report(
            MODULE.DEFAULT_EXECUTABLE.read_bytes(),
            json.loads(MODULE.DEFAULT_EVENT_IR.read_text(encoding="utf-8")),
        )
        self.assertEqual(report["operation"]["operationHex"], "0x003f")
        self.assertEqual(len(report["operation"]["routes"]), 2)
        self.assertEqual(report["allDiscInventory"]["provenCallCount"], 217)
        self.assertEqual(
