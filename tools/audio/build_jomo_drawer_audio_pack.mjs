...
if (!decodedDirectory || !existsSync(decodedDirectory)) {
  throw new Error(
    "Usage: node tools/audio/build_jomo_drawer_audio_pack.mjs "
...
if (
  evidence.schema !== "new-yokosuka-jomo-object-phase-callbacks-v3"
  || evidence.audioBoundary.policy !== "phase-labels-dynamically-proven"
) {
  throw new Error("JOMO drawer phase evidence is not runtime-closed");
}

const parsed = parseDTPK(bank);
const group = parsed.groups.find(
  ({ descriptorHex }) => descriptorHex === "a905",
);
...
function encodeWebMOpus(commandHex, input) {
  const temporaryDirectory = mkdtempSync(
    path.join(tmpdir(), "new-yokosuka-jomo-drawer-"),
  );
...
const tracks = definitions.map((definition) => {
  ...
  const sourceFilename = decodedFiles.find((candidate) => (
    candidate.includes(definition.marker)
    && !candidate.startsWith("fixed-")
    && candidate.toLowerCase().endsWith(".wav")
  ));
  if (!sourceFilename) {
    throw new Error(
      `Decoded F1OMOYAA sample ${definition.marker} was not found`,
    );
  }
  ...
  const output = encodeWebMOpus(definition.commandHex, pcm);
  writeFileSync(
...
const manifest = {
  schema: "new-yokosuka-jomo-drawer-audio-pack-v1",
  generatedBy: "tools/audio/build_jomo_drawer_audio_pack.mjs",
  source: {
    bank: "extracted_files/data/SCENE/01/SOUND/F1OMOYAA.SND",
    bankSha256: expectedBankHash,
    evidence: "tools/evidence/jomo-object-phase-callbacks.json",
  ...
  tracks,
  runtimeStatus: (
    "Wired to JOMO ATS1-ATS6 using the synchronized retail opening, "
    + "closing-start, and closing-impact dispatch phases."
  ),
};
writeFileSync(
  path.join(outputDirectory, "drawer-manifest.json"),
  `${JSON.stringify(manifest, null, 2)}\n`,
);
console.log(
  `Wrote ${tracks.length} JOMO drawer tracks to `
