...
const sources = [
    ...
    },
    {
        key: "pc-scene03",
        label: "PC SCENE/03",
        charaDir: ".disc-work/analysis/pc-sm1-ryo/SCENE/03/MODEL/CHARA",
        faceDir: ".disc-work/analysis/pc-sm1-ryo/SCENE/03/MODEL/FACE",
    },
];
...
function auditMt5(relPath) {
    if (!exists(relPath)) return null;
    const audit = runJson(["tools/assets/audit_mt5_model_state.js", relPath, "--json"]);
...
function atlasUse(relPath) {
    if (!exists(relPath)) return null;
    const atlas = runJson([
        "tools/assets/dump_mt5_atlas.js",
        relPath,
        "--texture",
        headAtlasHex,
        "--json",
...
function ftblReport(ftblRelPath, faceRelPath) {
    ...
    const args = ["tools/animation/dump_face_ftbl.js", ftblRelPath, "--json", "--limit", "4"];
...
function markdown(rows) {
    ...
    lines.push("");
    lines.push("Generated by `node tools/animation/build_ryo_asset_matrix.js`. Static file inspection only; no Windows executable is run.");
    lines.push("");
...
fs.mkdirSync(path.join(repoRoot, ".disc-work/analysis"), { recursive: true });
...
fs.writeFileSync(
    path.join(repoRoot, ".disc-work/analysis/ryo-asset-matrix.json"),
...
console.log(".disc-work/analysis/ryo-asset-matrix.json");
