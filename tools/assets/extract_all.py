...
Usage:
  python3 -m tools.assets.extract_all                    # Full pipeline
  python3 -m tools.assets.extract_all --skip-extract     # Skip GDI extraction (if already done)
  python3 -m tools.assets.extract_all --extract-only     # Only extract GDI, don't process
"""
    ...
    for label, disc_dir, extract_dir in discs:
        ...
        if gdi:
            ...
            if os.path.isdir(extract_dir):
                ...
                try:
                    subprocess.run([
                        sys.executable, '-m', 'tools.assets.gditools3', '-i', gdi,
                        '-o', extract_dir,
                        '--extract-all'
                    ], cwd=paths['project_dir'], check=True)
                    print(f"  Done.")
...
def step_sync_models(paths):
    ...
    subprocess.run([sys.executable, '-m', 'tools.assets.sync_models'], cwd=paths['project_dir'], check=True)
...
def step_sky_textures(paths):
    ...
    pvr_dir = os.path.join(paths['project_dir'], 'tools', 'extracted_tex')
    ...
    subprocess.run([sys.executable, '-m', 'tools.assets.convert_sky_textures'], cwd=paths['project_dir'], check=True)
