...
"""
    ...
    iso9660.py uses the upstream MIT-style permission grant, provided
    in tools/licenses/iso9660_license.txt. See tools/licenses/README.md.
"""
...
from tools.assets.iso9660 import ISO9660 as _ISO9660_orig
...
class CdImage():
    ...
    def __init__(self, filename, mode = 'auto', manualRawOffset=0, *args, **kwargs):
        ...
        if self.__mode in [2352, 2336]:
              self.length = (self.file.tell() - self.__manualRawOffset) * 2048//self.__mode
...
def parse_gdi(filename, verbose=False):
    ...
    gdi = [dict(filename=dirname + '/' + t[4], mode=int(t[3]), tnum=int(t[0]), lba=int(t[1]),
                manualRawOffset=int(t[5]) if len(t) > 5 else 0,
