...
const timeOfDayPresets = [
  ...
  {
    name: "Sunset",
    texture: "/textures/sky/air05.png",
    clearColor: [0.8, 0.4, 0.2, 1],
  },
  {
    name: "Evening",
    texture: "/textures/sky/air05.png",
    clearColor: [0.3, 0.2, 0.4, 1],
  },
  {
    name: "Night",
...
const createScene = () => {
  ...
  const skyLight = new BABYLON.DirectionalLight(
    "skyLight",
    new BABYLON.Vector3(0, -1, 0),
    scene,
  );
  ...
  const directLight = new BABYLON.DirectionalLight(
    "dirLight",
...
async function loadMetadata() {
  try {
    ...
    console.log("[Metadata] Loaded mappings", {
      ...
      chars: Object.keys(charNames).length,
    });
  } catch (err) {
...
function applyTimeOfDay(presetIndex) {
  ...
  if (preset.texture && !isInteriorScene) {
    currentSkybox = BABYLON.MeshBuilder.CreateSphere(
      "skyDome",
      {
        diameter: 50000,
        slice: 0.5,
