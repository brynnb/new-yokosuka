# Original game source examples — research reference

The preserved wiki-format material below describes original C source files left
on Shenmue game discs, including `0154_1.C` and `SEQCONV.C`; these are not New
Yokosuka source files or code reconstructed by this project. The imported text
retains its original `{{tcrf source|Ehm}}` credit to The Cutting Room Floor at
the bottom. Its exact imported page revision was not recorded.

This reference supports native event-camera structure research in
`tools/extract_event_camera_catalog.py`. Original code and quoted wiki material
retain their respective ownership; inclusion does not relicense them under the
project's GPL license. The wiki markup is intentionally preserved below.
