...
This is the project's older summary of the community-documented bytecode
family; the preserved instruction tables are in
[external references](../reference/external/scene-script-instructions.md).
It is not a universal decoder contract for every MAPINFO `SCN3` payload.
The current Dreamcast room-event recovery path analyzes compiled SH-4 code,
with exact relocation and control-flow evidence, as described in
[native script recovery](../guides/native-script-recovery.md). Do not apply
the byte-sized opcode table below to those executable regions merely because
their container has an `SCN3` signature. Earlier field interpretations below
must be checked against the exact source variant before reuse.

## Overview

SCN3 is the scene script format used in Shenmue 1 and 2. It contains:
- **Bytecode** in the community-described representation
